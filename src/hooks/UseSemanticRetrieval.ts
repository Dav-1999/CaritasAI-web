...
export default async function(messages:Message[]){
  const url = "https://api.caritas.pro/zhiPuAi/getSemanticRetrieval?question="+messages.slice(0,messages.length-1)[messages.slice(0,messages.length-1).length-1].text;
